...
function VerificationCode() {
  ...
  const verificationCodeError = useSelector(
    (state) => state.userSlice.verificationCodeError
  );
  const verificationCodeSuccess = useSelector(
    (state) => state.userSlice.verificationCodeSuccess
  );

  useEffect(() => {}, [verificationCodeError]);
    ...
    <>
      <GlobalLayout>
        <StartPageLayout
          ...
          headerTitle="Kartınızı Doğrulayın"
        >
          <div>
            {" "}
            Lütfen kartınız ile birlikte gelen 6 haneli doğrulama kodunu
            giriniz.{" "}
          </div>
          ...
          {verificationCodeError && (
            <div className="error-text">
              {" "}
              Girdiğiniz kod hatalı tekrar giriniz.{" "}
            </div>
          )}{" "}
          {verificationCodeSuccess && (
            <div className="success-text">
              {" "}
              Kartınız ile profiliniz başarılı bir şekilde eşleştirildi.{" "}
            </div>
          )}
