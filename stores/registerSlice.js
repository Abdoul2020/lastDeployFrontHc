...
export const addUserAsync = createAsyncThunk(
  "register/addUserAsync",
  async (data) => {
    const res = await axios
      .post(
        `https://us-central1-hibritardpro.cloudfunctions.net/api/register`,
        data
      )
      .then((res) => {
    ...
    const firstProfileRes = await axios
      .post(
        `https://us-central1-hibritardpro.cloudfunctions.net/api/addProfile`,
        firstProfileData
      )
      .then((res) => {
        ...
        axios
          .post(
            `https://us-central1-hibritardpro.cloudfunctions.net/api/clickProfile/${res.data.newProfileId}/add`
          )
          .then((res) => {});
  ...
  extraReducers: {
    ...
    [addUserAsync.fulfilled]: (state, action) => {
      if (action.payload.status == 400 || action.payload.status == 500) {
        ...
        state.status = "error";
        state.errors = {
          globalErrors: action.payload.data.allErrors,
          registeredEmail: action.payload.data.eMailAccountExist ? true : false,
        };
      } else {
