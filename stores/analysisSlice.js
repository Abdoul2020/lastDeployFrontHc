...
export const getAllDateofProfileClickAsync = createAsyncThunk(
  "analysis/getAllDateofProfileClickAsync",
  async (profileId) => {
    const res = await axios
      .get(
        `https://us-central1-hibritardpro.cloudfunctions.net/api/getAllDateofProfileClick/${profileId}`,
        {
          headers: {
...
export const clickProfileAsync = createAsyncThunk(
  "analysis/clickProfileAsync",
  async (profileId) => {
    const res = await axios
      .post(
        `https://us-central1-hibritardpro.cloudfunctions.net/api/clickProfile/${profileId}/add`
      )
      .then((res) => {
        return res;
      })
  ...
  extraReducers: {
    ...
    [getAllDateofProfileClickAsync.fulfilled]: (state, action) => {
      if (localStorage.getItem("GZIToken")) {
        state.clickDateList =
          action.payload.data &&
          action.payload.data.allUser &&
          action.payload.data.allUser.clickDate;
